...
웹도 두 페이지가 `PlaceForm`을 공유합니다. 내가 등록한 장소는 내 정보 탭에서,
또는 그 장소 상세 화면의 `수정` 버튼으로 들어갑니다.
...
- 저장에 성공하면 폼으로 돌아오지 않고 그 장소 상세로 보냅니다. 무엇이 저장됐는지 바로
  확인할 수 있어야 합니다. 다만 **상세 화면에서 수정으로 들어온 경우에는 상세를 새로
  쌓지 않고 폼만 걷어냅니다**(`Get.previousRoute`로 판정). 또 쌓으면 뒤로 가기가 같은
  화면을 두 번 지나가고, 아래에 깔린 쪽은 낡은 값을 들고 있습니다. 돌아간 상세는
  열어 준 쪽이 다시 받습니다.
- 중복(같은 유형·이름이 근처에 있음)·권한(남의 장소)·주소 실패는 서버가 한글 메시지로
  주므로 그대로 보여 줍니다.
- **수정 권한은 작성자 고유 번호(`createdByUserId`)로만 판정합니다.** 사용자명은 바뀔 수
  있고, 서버도 같은 값으로 검사합니다(`PlaceService.requireOwnedBy`). 그래서 상세 화면의
  `수정` 버튼이 보이는 조건과 서버가 허용하는 조건이 어긋나지 않습니다.
- 남의 장소로 수정 화면에 들어오면 **폼을 그리기 전에 잠급니다.** 서버가 어차피 403을
  주지만, 그러면 다 채워 넣은 다음에야 막힌 걸 알게 됩니다.
